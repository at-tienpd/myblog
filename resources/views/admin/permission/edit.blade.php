...
@section('content')
        <div class="page-header">
            <h1>{{ trans('permission.page_header.edit') }} {{ $permission -> display_name }}</h1>
        </div>
        <p>{{ trans('permission.description.edit') }}</p>
        ...
        <table class="table table-hover table-bordered">
                <caption>{{ trans('permission.table_caption.edit') }}</caption>
                <thead>
                    <tr>
                        <th>#</th>
        ...
        {{ method_field('PUT') }}
        <fieldset>
            <legend>{{ trans('permission.legend_form.edit') }}</legend>
            <div class="form-group has-warning">
                <label for="name">{{ trans('permission.field_name.name') }}</label>
                <input type="text" class="form-control" id="name" placeholder="Name" name="name" required="required" value="{{ $permission -> name }}">
                <span id="name" class="help-block">{{ trans('permission.warning.update') }}</span>
                @if ($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="display_name" placeholder="Display name" name="display_name" required="required" value="{{ $permission -> display_name }}">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="description" placeholder="Description" name="description" required="required" value="{{ $permission -> description }}">
                @if ($errors->has('description'))
                     <small class=error>{{ $errors->first('description', ':message') }}</small>
                @endif
            </div>
            <button class="btn btn-default" type="submit">{{ trans('permission.button.update') }}</button>
            </fieldset>
        </form>
@stop